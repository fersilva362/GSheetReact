...
function App() {
    ...
    fetch(
      ...
      .then((data) => {
        const array = data
          ...
          .slice(1)
          .map((row) => {
            const [date, players_1, players_2, goals_T1, goals_T2] =
              row.split("\t");
            return {
              date,
              players_1,
              players_2,
              goals_T1: parseInt(goals_T1),
              goals_T2: parseInt(goals_T2),
            };
          });
        setMatch(array);
      });
  ...
  if (match.length !== 0) {
    for (const { players_1, players_2, goals_T1, goals_T2 } of match) {
      ...
      for (let name of names1) {
        ...
        player.match++;
        player.goals += goals_T1 - goals_T2;
      ...
      for (let name of names2) {
        ...
        let player = players.get(name) || { name: name, match: 0, goals: 0 };
        player.match++;
        player.goals += goals_T2 - goals_T1;
        players.set(name, player);
      }
    }
  }
  const playersArr = Array.from(players.values())
    ...
    .map((player) => ({
      ...player,
      score: Math.round(player.goals / player.match),
    }));
  //
